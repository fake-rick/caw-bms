...
int mpu_reg_dump(void) {
  ...
  for (ii = 0; ii < st.hw->num_reg; ii++) {
    ...
    if (i2c_read(st.hw->addr, ii, 1, &data)) return -1;
    log_i("%#5x: %#5x\r\n", ii, data);
  }
  return 0;
}
...
int mpu_read_reg(unsigned char reg, unsigned char *data) {
  if (reg == st.reg->fifo_r_w || reg == st.reg->mem_r_w) return -1;
  if (reg >= st.hw->num_reg) return -1;
 ...
 *  @return     0 if successful.
 */
int mpu_init(I2C_HandleTypeDef *hi2c, struct int_param_s *int_param) {
  unsigned char data[6], rev;

  G_MPU_DEF.i2c_ins = hi2c;
  ...
  if (i2c_read(st.hw->addr, st.reg->accel_offs, 6, data)) return -1;
  ...
  if (rev) {
    ...
    if (i2c_read(st.hw->addr, st.reg->prod_id, 1, data)) return -1;
    rev = data[0] & 0x0F;
    if (!rev) {
      log_e(
...
#define MPU6500_MEM_REV_ADDR (0x17)
  if (mpu_read_mem(MPU6500_MEM_REV_ADDR, 1, &rev)) return -1;
  if (rev == 0x1)
    st.chip_cfg.accel_half = 0;
  else {
    log_e("Unsupported software product rev %d.\n", rev);
  ...
  if (i2c_write(st.hw->addr, st.reg->accel_cfg2, 1, data)) return -1;
#endif
...
int mpu_lp_accel_mode(unsigned char rate) {
  ...
  if (!rate) {
    ...
    tmp[0] = 0;
    tmp[1] = BIT_STBY_XYZG;
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 2, tmp)) return -1;
    st.chip_cfg.lp_accel_mode = 0;
    return 0;
  }
  ...
  if (rate == 1)
    ...
    tmp[0] = INV_LPA_640HZ;
  if (i2c_write(st.hw->addr, st.reg->lp_accel_odr, 1, tmp)) return -1;
  tmp[0] = BIT_LPA_CYCLE;
  if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, tmp)) return -1;
#endif
...
int mpu_get_gyro_reg(short *data, unsigned long *timestamp) {
  ...
  if (!(st.chip_cfg.sensors & INV_XYZ_GYRO)) return -1;

  if (i2c_read(st.hw->addr, st.reg->raw_gyro, 6, tmp)) return -1;
...
int mpu_get_accel_reg(short *data, unsigned long *timestamp) {
  ...
  if (!(st.chip_cfg.sensors & INV_XYZ_ACCEL)) return -1;

  if (i2c_read(st.hw->addr, st.reg->raw_accel, 6, tmp)) return -1;
  data[0] = (tmp[0] << 8) | tmp[1];
  data[1] = (tmp[2] << 8) | tmp[3];
  data[2] = (tmp[4] << 8) | tmp[5];
  if (timestamp) get_ms(timestamp);
...
int mpu_get_temperature(long *data, unsigned long *timestamp) {
  ...
  if (i2c_read(st.hw->addr, st.reg->temp, 2, tmp)) return -1;
  raw = (tmp[0] << 8) | tmp[1];
  if (timestamp) get_ms(timestamp);
...
int mpu_set_accel_bias(const long *accel_bias) {
  ...
  if (!accel_bias) return -1;
  if (!accel_bias[0] && !accel_bias[1] && !accel_bias[2]) return 0;

  if (i2c_read(st.hw->addr, 3, 3, data)) return -1;
  fg[0] = ((data[0] >> 4) + 8) & 0xf;
  fg[1] = ((data[1] >> 4) + 8) & 0xf;
  fg[2] = ((data[2] >> 4) + 8) & 0xf;
  ...
  if (i2c_read(st.hw->addr, 0x06, 6, data)) return -1;
  ...
  if (i2c_write(st.hw->addr, 0x06, 6, data)) return -1;
  return 0;
}
...
int mpu_get_accel_fsr(unsigned char *fsr) {
  ...
  if (st.chip_cfg.accel_half) fsr[0] <<= 1;
  return 0;
}
...
int mpu_set_compass_sample_rate(unsigned short rate) {
  ...
  if (i2c_write(st.hw->addr, st.reg->s4_ctrl, 1, &div)) return -1;
...
int mpu_set_sensors(unsigned char sensors) {
  ...
  if (sensors & INV_XYZ_COMPASS)
    ...
    mpu_set_bypass(0);
#else
  if (i2c_read(st.hw->addr, st.reg->user_ctrl, 1, &user_ctrl)) return -1;
  ...
  if (st.chip_cfg.dmp_on)
    ...
    user_ctrl &= ~BIT_DMP_EN;
  if (i2c_write(st.hw->addr, st.reg->s1_do, 1, &data)) return -1;
  /* Enable/disable I2C master mode. */
  if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, &user_ctrl)) return -1;
#endif
#endif
...
int mpu_get_int_status(short *status) {
  unsigned char tmp[2];
  if (!st.chip_cfg.sensors) return -1;
  if (i2c_read(st.hw->addr, st.reg->dmp_int_status, 2, tmp)) return -1;
...
int mpu_read_fifo(short *gyro, short *accel, unsigned long *timestamp,
  ...
  if (fifo_count > (st.hw->max_fifo >> 1)) {
    ...
    if (i2c_read(st.hw->addr, st.reg->int_status, 1, data)) return -1;
  ...
  if (i2c_read(st.hw->addr, st.reg->fifo_r_w, packet_size, data)) return -1;
...
int mpu_read_fifo_stream(unsigned short length, unsigned char *data,
  ...
  if (!st.chip_cfg.dmp_on) {
    CAW_LOG_ERROR("!st.chip_cfg.dmp_on error");
    return -1;
  }
  if (!st.chip_cfg.sensors) {
    CAW_LOG_ERROR("!st.chip_cfg.sensors error");
    return -1;
  }

  if (i2c_read(st.hw->addr, st.reg->fifo_count_h, 2, tmp)) {
    CAW_LOG_ERROR("i2c_read failed");
    return -1;
  }
  fifo_count = (tmp[0] << 8) | tmp[1];
  if (fifo_count < length) {
    more[0] = 0;
    CAW_LOG_ERROR("fifo_count < length error");
    return -1;
  }
  if (fifo_count > (st.hw->max_fifo >> 1)) {
    /* FIFO is 50% full, better check overflow bit. */
    ...
    if (tmp[0] & BIT_FIFO_OVERFLOW) {
      mpu_reset_fifo();
      CAW_LOG_ERROR("tmp[0] & BIT_FIFO_OVERFLOW error");
      return -2;
    }
  }
...
static int compass_self_test(void) {
  ...
  if (i2c_write(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp)) return 0x07;
  ...
  do {
    ...
    if (i2c_read(st.chip_cfg.compass_addr, AKM_REG_ST1, 1, tmp))
      goto AKM_restore;
    if (tmp[0] & AKM_DATA_READY) break;
  } while (tries--);
  if (!(tmp[0] & AKM_DATA_READY)) goto AKM_restore;

  if (i2c_read(st.chip_cfg.compass_addr, AKM_REG_HXL, 6, tmp)) goto AKM_restore;

  result = 0;
  data = (short)(tmp[1] << 8) | tmp[0];
  if ((data > 100) || (data < -100)) result |= 0x01;
  data = (short)(tmp[3] << 8) | tmp[2];
  if ((data > 100) || (data < -100)) result |= 0x02;
  data = (short)(tmp[5] << 8) | tmp[4];
  if ((data > -300) || (data < -1000)) result |= 0x04;
...
int mpu_run_self_test(long *gyro, long *accel) {
  ...
  for (ii = 0; ii < tries; ii++)
    if (!get_st_biases(gyro, accel, 0)) break;
  if (ii == tries) {
  ...
  for (ii = 0; ii < tries; ii++)
    if (!get_st_biases(gyro_st, accel_st, 1)) break;
  if (ii == tries) {
...
int mpu_write_mem(unsigned short mem_addr, unsigned short length,
  ...
  if (i2c_write(st.hw->addr, st.reg->bank_sel, 2, tmp)) return -1;
  if (i2c_write(st.hw->addr, st.reg->mem_r_w, length, data)) return -1;
  return 0;
}
...
int mpu_read_mem(unsigned short mem_addr, unsigned short length,
  ...
  if (i2c_write(st.hw->addr, st.reg->bank_sel, 2, tmp)) return -1;
  if (i2c_read(st.hw->addr, st.reg->mem_r_w, length, data)) return -1;
  return 0;
}
...
#define LOAD_CHUNK (16)
  ...
  if (st.chip_cfg.dmp_loaded) /* DMP should only be loaded once. */
    return -1;
  ...
  if (i2c_write(st.hw->addr, st.reg->prgm_start_h, 2, tmp)) return -1;
...
static int setup_compass(void) {
  ...
  for (akm_addr = 0x0C; akm_addr <= 0x0F; akm_addr++) {
    ...
    if (!result && (data[0] == AKM_WHOAMI)) break;
  }
  ...
  if (i2c_write(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, data)) return -1;
  delay_ms(1);
  ...
  if (i2c_read(st.chip_cfg.compass_addr, AKM_REG_ASAX, 3, data)) return -1;
  ...
  if (i2c_write(st.hw->addr, st.reg->s0_addr, 1, data)) return -1;

  /* Compass reads start at this register. */
  data[0] = AKM_REG_ST1;
  if (i2c_write(st.hw->addr, st.reg->s0_reg, 1, data)) return -1;

  /* Enable slave 0, 8-byte reads. */
  data[0] = BIT_SLAVE_EN | 8;
  if (i2c_write(st.hw->addr, st.reg->s0_ctrl, 1, data)) return -1;

  /* Slave 1 changes AKM measurement mode. */
  data[0] = st.chip_cfg.compass_addr;
  if (i2c_write(st.hw->addr, st.reg->s1_addr, 1, data)) return -1;

  /* AKM measurement mode register. */
  data[0] = AKM_REG_CNTL;
  if (i2c_write(st.hw->addr, st.reg->s1_reg, 1, data)) return -1;

  /* Enable slave 1, 1-byte writes. */
  data[0] = BIT_SLAVE_EN | 1;
  if (i2c_write(st.hw->addr, st.reg->s1_ctrl, 1, data)) return -1;
  ...
  data[0] = 0x03;
  if (i2c_write(st.hw->addr, st.reg->i2c_delay_ctrl, 1, data)) return -1;

#ifdef MPU9150
  /* For the MPU9150, the auxiliary I2C bus needs to be set to VDD. */
  data[0] = BIT_I2C_MST_VDDIO;
  if (i2c_write(st.hw->addr, st.reg->yg_offs_tc, 1, data)) return -1;
#endif
...
int mpu_get_compass_reg(short *data, unsigned long *timestamp) {
  ...
  if (!(st.chip_cfg.sensors & INV_XYZ_COMPASS)) return -1;

#ifdef AK89xx_BYPASS
  if (i2c_read(st.chip_cfg.compass_addr, AKM_REG_ST1, 8, tmp)) return -1;
  tmp[8] = AKM_SINGLE_MEASUREMENT;
  if (i2c_write(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp + 8)) return -1;
#else
  if (i2c_read(st.hw->addr, st.reg->raw_compass, 8, tmp)) return -1;
#endif

#if defined AK8975_SECONDARY
  /* AK8975 doesn't have the overrun error bit. */
  if (!(tmp[0] & AKM_DATA_READY)) return -2;
  if ((tmp[7] & AKM_OVERFLOW) || (tmp[7] & AKM_DATA_ERROR)) return -3;
#elif defined AK8963_SECONDARY
  /* AK8963 doesn't have the data read error bit. */
  if (!(tmp[0] & AKM_DATA_READY) || (tmp[0] & AKM_DATA_OVERRUN)) return -2;
  if (tmp[7] & AKM_OVERFLOW) return -3;
#endif
...
int mpu_lp_motion_interrupt(unsigned short thresh, unsigned char time,
  ...
  if (lpa_freq) {
    ...
    if (!time) /* Minimum duration must be 1ms. */
      time = 1;
    ...
    if (i2c_write(st.hw->addr, st.reg->user_ctrl, 3, data)) goto lp_int_restore;
...
lp_int_restore:
  ...
  if (st.chip_cfg.cache.dmp_on) mpu_set_dmp_state(1);

#ifdef MPU6500
  /* Disable motion interrupt (MPU6500 version). */
  data[0] = 0;
  if (i2c_write(st.hw->addr, st.reg->accel_intel, 1, data)) goto lp_int_restore;
#endif
